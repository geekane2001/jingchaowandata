...
# =========================================================
# === 核心修改：使用最健壮的 lifespan 启动方式 ===
# =========================================================
@asynccontextmanager
async def lifespan_manager(app: FastAPI):
    # 应用启动时执行
    logging.info("接收到 'lifespan.startup' 事件，正在启动后台抓取任务...")
    # 创建任务，并将其保存在 app_state 中，以便在关闭时可以取消它
    ...
    if app_state["task"]:
        ...
        try:
            await app_state["task"]
        except asyncio.CancelledError:
            logging.info("后台任务已成功取消。")

# --- FastAPI 应用实例和路由 ---
app = FastAPI(lifespan=lifespan_manager) # 将 lifespan 管理器应用到 FastAPI 实例
